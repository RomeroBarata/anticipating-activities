...
class ModelCNN:
    ...
    def __init__(self, nRows, nCols, nClasses=None):
        self.nRows = nRows
        self.nCols = nCols
        self.nClasses = nClasses if nClasses is not None else nCols
        self.input_vid = tf.placeholder('float', [None, self.nRows, self.nCols, 1], name='input_vid')
        self.target = tf.placeholder('float', [None, self.nRows, self.nClasses, 1], name='target')
    ...
    def __build(self):
        ...
        W_fc2 = self.__weight_variable([1024, self.nRows * self.nClasses], 'W_fc2')
        b_fc2 = self.__bias_variable([self.nRows * self.nClasses], 'b_fc2')
        ...
        prediction_unscaled = tf.reshape(pred_flat, [-1, self.nRows, self.nClasses, 1])
        # l2 normalization
        ...
        for epoch in range(nEpochs):
            ...
            while(batch_gen.has_next()):
                ...
                _, acc = sess.run([optimizer, accuracy],
                                  feed_dict={self.input_vid: batch_vid, self.target: batch_target})
                i = i + 1
                epoch_acc += acc
            batch_gen.reset()
    ...
    def predict(self, sess, model_save_path, input_x, sigma, actions_dict):
        ...
        result = np.reshape(result, [self.nRows, self.nClasses])
